...
import { announceGrade, createTaskReportController, TasksPanel, type Task, type TaskLoadStatus } from "./tasks";
...
export function SessionShellView(props: { sessionId: string; step: SessionStep }) {
  ...
  const taskReports = createTaskReportController({
    ...
    setReport: (index, report) => setState("tasks", index, "report", report),
  });
  ...
  async function loadTasks(sessionID: string, seq: number, controller: AbortController) {
    try {
      ...
      taskReports.pollAll(data.tasks);
    } catch (error) {
      if (!isAbort(error) && isCurrentLoad(seq, controller)) {
        setState("tasksStatus", "error");
      }
